...
function busicol_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'busicol' ) . '</span></li>';
...
// Blog categories
function busicol_featured_post_cat(){
...
// social media
if ( ! function_exists( 'busicol_social' ) ) {
	function busicol_social( $args = array() ) {
		...
		$url = busicol_opt( 'busicol_social_url' );
...
// header cart count
function busicol_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( BUSICOL_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'busicol' ); ?>">
...
function busicol_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'busicol_contact7_form_content', 10, 2 );