...
function busicol_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'busicol' ),
        'next_string'     => esc_html__( '&raquo;', 'busicol' ),
        'before_output'   => '<div class="busicol-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'busicol_pagination_defaults', $defaults )
    );
